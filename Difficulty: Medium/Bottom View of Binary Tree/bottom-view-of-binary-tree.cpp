...
class Solution {
  public:
    vector<int> bottomView(Node *root) {
        // -------------------- Algorithm --------------------
        // Bottom View = nodes visible when looking from below.
        // Idea:
        ...
        // 3. For each hd, record the LAST node encountered (bottommost).
        //    → overwrite values in map each time.
        ...
        while(!q.empty()){
            ...
            // For bottom view: overwrite every time (last node at this hd)
            mp[it] = temp->data;
        ...
        return ans;                       // return bottom view list
    }
};
...
- Node 4 (hd=0) → overwrite mp[0]=4
- Node 5 (hd=+1) → overwrite mp[+1]=5
- Node 6 (hd=+2) → mp[+2]=6

Final map = { -1:2, 0:4, +1:5, +2:6 }
Bottom View = [2, 4, 5, 6]
...
-------------------- Key Notes --------------------
- BFS ensures we process nodes level by level.
- map<int,int> keeps hd sorted automatically → left to right view.
- For bottom view, we overwrite values at each hd (last node wins).
